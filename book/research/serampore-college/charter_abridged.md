STATUS: DONE (partial — see note below)
TICKET: B-serampore-college
ROLE: Abridger
INPUTS READ: text/marshman1859-v2.txt; text/smith-carey-1885.txt; dossier.md field F5; book/plan/01_book_design.md section 6

# Abridged charter — Serampore College (`serampore-college`)

**Source document:** *Prospectus of a College for the Instruction of Asiatic, Christian, and
Other Youth in Eastern Literature and European Science*, issued 15 July 1818, English.
document_id `marshman1859-v2`.

**Note on the 1827 Danish Royal Charter.** The roster's F5 field also names the 1827 Danish
Royal Charter of Incorporation as a founding document. Because it survives here only as a
quotation embedded in a *different* secondary work (George Smith, `smith-carey-1885`) from the
one used for the Prospectus, and `check_quotes.py --charter` checks a single named document per
file, the Charter's terms are treated in the chapter's Founding section (with its own footnote
to `smith-carey-1885`) rather than folded into this file. This is a mechanical, not a
substantive, separation — see dossier.md field C8 for the Charter's degree-granting clause,
quoted in full there.

**Translation:** none needed (English-language document).

**IMPORTANT — scope of this abridgement.** The Prospectus was not located as a digitized full
text and fetched this session (see discrepancies.md, item 3). It survives here only as quoted
fragments embedded in a secondary work: John Clark Marshman's 1859 family memoir, which
paraphrases the Prospectus closely and quotes several of its clauses directly in quotation
marks. This is the same "charter-in-substance" fallback used in the `geneva-academy` chapter.
Every quoted sentence below is verbatim against `text/marshman1859-v2.txt` and passes
`check_quotes.py --charter`. Passages that are Marshman's own paraphrase, not a direct
quotation, are so marked and are not set off as quoted charter text.

## Structure of this partial abridgement

| # | Content | Source (quote_id) | KEEP/CUT |
|---|---|---|---|
| 1 | Title and date of the Prospectus (15 July 1818) | serampore-college-q001 | KEEP (a) |
| 2 | Stated purpose: "the handmaid of evangelisation" | serampore-college-q002 | KEEP (a) |
| 3 | Doctrinal argument: native must answer native | serampore-college-q003 | KEEP (b) |
| 4 | On language: vernacular first, English for advanced students | serampore-college-q004, serampore-college-q005 | KEEP (d) |
| 5 | Admission: open to all castes and creeds; unsectarian instruction | serampore-college-q007, serampore-college-q008 | KEEP (c) |
| 6 | The three objects of the College (non-resident heathen; resident Christian; native missionary) | serampore-college-q020 | KEEP (a) |
| 7 | The theological division: Divinity Professor, piety required, four-year course | serampore-college-q014, serampore-college-q016, serampore-college-q017, serampore-college-q019 | KEEP (c, d, g) |
| — | The Danish Royal Charter's degree-granting clause (1827) | serampore-college-q012 (quoted from a different source, `smith-carey-1885`; treated in the chapter's Founding section, not here) | see note above |
| — | The Prospectus's own full text (administrative/financial articles: budget table, trustee/property clauses) | not located this session | CUT — NOT FOUND, see discrepancies.md |

## Headnote

*Prospectus of a College for the Instruction of Asiatic, Christian, and Other Youth in Eastern
Literature and European Science*, issued by William Carey, Joshua Marshman and William Ward at
Serampore, 15 July 1818, English; and the Danish Royal Charter of Incorporation granted by King
Frederick VI, 1827, English/Danish; both quoted here at second hand from John Clark Marshman,
*The Life and Times of Carey, Marshman and Ward* (London, 1859) and George Smith, *The Life of
William Carey* (London, 1885).

## Text

[Title and occasion.] On 15 July 1818, the missionaries "issued the prospectus of a" College
for the instruction of Asiatic, Christian, and other youth [in Eastern literature and European
science]. It was drawn up by Dr. Marshman, in his peculiar and argumentative style, and
announced in unequivocal terms that the institution was intended to be the handmaid of
evangelisation.

[...]

[On the College's necessity and purpose, in the founders' own words as quoted by Marshman:]

> "If ever...the Gospel stands in India, it must be by native opposed to native in demonstrating
> its excellence above [all other systems]."

[...]

[On languages of instruction:]

> "Though it would be vain to attempt to enlighten a country through the medium of any language
> besides its own, it does not follow that English cannot be studied as a learned language, to
> great advantage, by youths of superior talent" — so that a select number of advanced students
> might be enabled "to dive into the deepest recesses of European science, and enrich their own
> language with its choicest treasures."

[...]

[On admission.] The institution was to be open to native youths from all parts of India,
without distinction of caste or creed. Christian youths of Asiatic parentage, of every
denomination, were to be admitted into it, with the understanding that the instruction should
be divested of everything of a sectarian character.

[...]

[On the College's three objects, from Carey's own 1826 letter to the Baptist Missionary Society
committee describing the institution he, Marshman and Ward had founded:]

> "Of the three objects connected with the College, the education of non-resident heathen
> students, the education of resident Christian students, and the preparation of missionaries
> from those born in the country, the first is not strictly a missionary object, the two latter
> are intimately connected with the progress of the good cause."

[...]

[On the theological division, again in Carey's words:]

> "To secure an increase of missionaries in European habits we have formed a class of
> theological students in the college, under the Divinity Professor. It contains at present six
> promising youths, of whose piety we have in some cases undoubted evidence, in others
> considerable ground for hope. The class will shortly be increased to twelve, but none will be
> continued in it who do not manifest undeniable piety and devotedness to the cause of missions.
> As we propose to allow each student to remain on an average four years, we may calculate upon
> the acquisition of two, and perhaps three, additional labourers annually."

[...]

[The Prospectus text recovered this session ends here. The Danish Royal Charter's own
degree-granting clause, granted nine years later in 1827, is quoted in full in dossier.md field
C8 and discussed in the chapter's Founding section, from a different secondary source
(`smith-carey-1885`) than the one used above.]
